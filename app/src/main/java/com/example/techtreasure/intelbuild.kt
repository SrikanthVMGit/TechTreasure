...
class intelbuild : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_intelbuild)
        ...
        // Arrays for prices
        val icpuPrices = resources.getStringArray(R.array.icpu_prices)
        val igpuPrices = resources.getStringArray(R.array.igpu_prices)
        ...
        spinner1.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
            override fun onItemSelected(parent: AdapterView<*>?, view: View?, position: Int, id: Long) {
                ...
                if (selectedItem != "Processors") {
                    ...
                    val price = icpuPrices[position - 1] // Adjust index because of the header item
                    totalPrice += price.substring(1).replace(",", "").toInt()
                    totalPriceTextView.text = "Total Price: ₹$totalPrice"
                    // Display corresponding image
                    if (selectedItem == "Intel i5") {
                        imageView1.setImageResource(R.drawable.i5)
                    } else if (selectedItem == "Intel i7") {
                        imageView1.setImageResource(R.drawable.i7)
                    } else if (selectedItem == "Intel i9") {
                        imageView1.setImageResource(R.drawable.i9)
                    }
                    // Show toast with selected item and price
                    Toast.makeText(this@intelbuild, "$selectedItem - Price: $price", Toast.LENGTH_SHORT).show()
                } else if (selectedItem == "Ram") {
                    // Display default image when "Ram" is selected
                    imageView3.setImageResource(R.drawable.ram)
                }
            }
        ...
        spinner2.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
            override fun onItemSelected(parent: AdapterView<*>?, view: View?, position: Int, id: Long) {
                ...
                if (selectedItem != "Graphics Cards") {
                    // Fetch price based on selected item position
                    val price = igpuPrices[position - 1] // Adjust index because of the header item
                    totalPrice += price.substring(1).replace(",", "").toInt()
                    totalPriceTextView.text = "Total Price: ₹$totalPrice"
                    // Display corresponding image
                    if (selectedItem == "GTX 1660") {
                        imageView2.setImageResource(R.drawable.g1660)
                    } else if (selectedItem == "RTX 3050") {
                        imageView2.setImageResource(R.drawable.g3050)
                    } else if (selectedItem == "RTX 4080") {
                        imageView2.setImageResource(R.drawable.g4080)
                    }
                    // Show toast with selected item and price
                    Toast.makeText(this@intelbuild, "$selectedItem - Price: $price", Toast.LENGTH_SHORT).show()
                }
